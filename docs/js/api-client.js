...
class AINovelAPI {
    ...
    constructor(baseUrl) {
      ...
      // URLのクエリパラメータにdebug=trueがある場合はデモモードを有効化
      const urlParams = new URLSearchParams(window.location.search);
      this.debugMode = urlParams.get('debug') === 'true';
      
      // GitHub Pagesのデモンストレーション用（APIサーバーがない場合）
      if (this.debugMode) {
        console.log('デモモードが有効です。APIサーバーなしで動作します。');
      }
    }
    ...
    async sendRequest(endpoint, data) {
      try {
        // デモモードの場合はモックレスポンスを返す
        if (this.debugMode) {
          if (endpoint === '/api/ideas') {
            return await this.getMockResponse('ideas');
          } else if (endpoint === '/api/generate') {
            return await this.getMockResponse('generate');
          } else if (endpoint === '/api/continue') {
            return await this.getMockResponse('continue');
          }
        }
        ...
        // APIサーバーに接続できない場合、デモモードを使用
        if (error.message.includes('Failed to fetch') || error.message.includes('NetworkError')) {
          console.log('APIサーバーに接続できないため、デモモードを使用します。');
          if (endpoint === '/api/ideas') {
            return await this.getMockResponse('ideas');
          } else if (endpoint === '/api/generate') {
            return await this.getMockResponse('generate');
